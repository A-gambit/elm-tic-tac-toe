...
Elm.Game.Update.make = function (_elm) {
   ...
   var checkWin = F2(function (cur,table) {
      ...
      return A2($List.any,
      function (row) {
         return _U.eq($List.sum(row),A2(F2(function (x,y) {    return x * y;}),cur,$List.length(row)));
      },
      table) || (A2($List.any,function (index) {    return A2($List.all,checkItemInRow(index),table);},_U.range(0,$List.length(table))) || (A2($List.all,
      checkItem,
