...
Elm.Game.Model.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var initialModel = {table: _U.list([_U.list([0,0,0]),_U.list([0,0,0]),_U.list([0,0,0])]),cur: 1,winner: 0,draw: false};
   var Model = F4(function (a,b,c,d) {    return {table: a,cur: b,winner: c,draw: d};});
   return _elm.Game.Model.values = {_op: _op,Model: Model,initialModel: initialModel};
};
Elm.Html = Elm.Html || {};
...
Elm.Game.Update.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var checkWin = F2(function (cur,table) {
      var getItem = F2(function (index,row) {    return $List.head(A2($List.drop,index,A2($List.take,index + 1,row)));});
      var getReverseItem = F2(function (index,row) {    return A2(getItem,$List.length(row) - index - 1,row);});
      var checkRow = function (row) {    return _U.eq($List.sum(row),A2(F2(function (x,y) {    return x * y;}),cur,$List.length(row)));};
      var checkItem = function (val) {    return _U.eq(val,$Maybe.Just(cur));};
      var checkItemInRow = F2(function (index,row) {    return checkItem(A2(getItem,index,row));});
      return A2($List.any,checkRow,table) || (A2($List.any,
      function (index) {
         return A2($List.all,checkItemInRow(index),table);
      },
      _U.range(0,$List.length(table))) || (A2($List.all,checkItem,A2($List.indexedMap,getItem,table)) || A2($List.all,
      checkItem,
      A2($List.indexedMap,getReverseItem,table))));
   });
...
Elm.Game = Elm.Game || {};
Elm.Game.Style = Elm.Game.Style || {};
Elm.Game.Style.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.Style = _elm.Game.Style || {};
   if (_elm.Game.Style.values) return _elm.Game.Style.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var itemStyle = _U.list([{ctor: "_Tuple2",_0: "line-height",_1: "60px"}
   ...
   var win = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "inline-block"}
                                            ,{ctor: "_Tuple2",_0: "color",_1: "#5a5b5f"}
                                            ,{ctor: "_Tuple2",_0: "font-size",_1: "18px"}
                                            ,{ctor: "_Tuple2",_0: "margin-right",_1: "15px"}]));
   var restart = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "inline-block"}
                                                ,{ctor: "_Tuple2",_0: "color",_1: "#acacae"}
                                                ,{ctor: "_Tuple2",_0: "font-size",_1: "18px"}
                                                ,{ctor: "_Tuple2",_0: "cursor",_1: "pointer"}]));
   var sub = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]));
   var title = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "block"}
                                              ,{ctor: "_Tuple2",_0: "margin",_1: "40px 0 20px 0"}
                                              ,{ctor: "_Tuple2",_0: "color",_1: "#5a5b5f"}
                                              ,{ctor: "_Tuple2",_0: "text-align",_1: "center"}
                                              ,{ctor: "_Tuple2",_0: "font-size",_1: "32px"}]));
   var content = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "backgroundColor",_1: "#fff"}
                                                ,{ctor: "_Tuple2",_0: "height",_1: "500px"}
                                                ,{ctor: "_Tuple2",_0: "overflow",_1: "hidden"}
                                                ,{ctor: "_Tuple2",_0: "width",_1: "100%"}]));
   return _elm.Game.Style.values = {_op: _op
                                   ,content: content
                                   ,title: title
                                   ,sub: sub
                                   ,restart: restart
...
Elm.Game.View.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Game$Model = Elm.Game.Model.make(_elm),
   $Game$Style = Elm.Game.Style.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var itemEl = F4(function (address,i,j,val) {
      var action = _U.eq(val,0) ? A2($Game$Update.Move,i,j) : $Game$Update.NoOp;
      var toSymbol = function (val) {
         var _p0 = val;
         switch (_p0)
   ...
   var tableEl = F2(function (address,table) {
      var content = A2($List.indexedMap,rowEl(address),table);
      return A2($Html.div,_U.list([$Game$Style.table]),content);
   });
   var winnerEl = F3(function (address,winner,draw) {
      var showWinner = function (val) {
         return A2(F2(function (x,y) {    return A2($Basics._op["++"],x,y);}),_U.eq(val,1) ? "First" : "Second"," player win");
      };
      var result = !_U.eq(winner,0) ? showWinner(winner) : draw ? "Draw" : "";
      return A2($Html.div,
      _U.list([$Game$Style.sub]),
      _U.list([A2($Html.span,_U.list([$Game$Style.win]),_U.list([$Html.text(result)]))
              ,A2($Html.span,_U.list([A2($Html$Events.onClick,address,$Game$Update.Restart),$Game$Style.restart]),_U.list([$Html.text("Restart")]))]));
   });
   var title = A2($Html.div,_U.list([$Game$Style.title]),_U.list([A2($Html.span,_U.list([]),_U.list([$Html.text("Tic-Tac-Toe!")]))]));
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([$Game$Style.content]),_U.list([title,A3(winnerEl,address,model.winner,model.draw),A2(tableEl,address,model.table)]));
   });
   return _elm.Game.View.values = {_op: _op,view: view,title: title,winnerEl: winnerEl,tableEl: tableEl,rowEl: rowEl,itemEl: itemEl};
};
Elm.Game = Elm.Game || {};
Elm.Game.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $Game$Model.initialModel,view: $Game$View.view,update: $Game$Update.update});
   return _elm.Game.values = {_op: _op,main: main};
};
